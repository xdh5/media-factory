"""语言学习成片发布：中文发 YouTube Daily Chinese Learning，韩语由 Agent 用矩媒 MCP 发到「韩语」。"""
...
from core.tools.meta import MetaToolError, publish_meta_reels
from core.tools.storage import StorageError, upload_public_file
...
def attach_publish_manifest(video_result: dict, words_by_mode: dict) -> dict:
    """给成片补上发布清单。中文确认后发 YouTube 与 Facebook/Instagram Reels；韩语发矩媒。"""
    ...
    for video in video_result.get("videos") or []:
        ...
        if mode == "en-zh":
            ...
            youtube_items.append({
                ...
                "short_title": f"中文{topic or '单词'}怎么说",
        ...
        if mode == "en-ko":
            ...
            matrixmedia_items.append({
                "learning_mode": "en-ko",
                "account_group": KOREAN_PUBLISH_ACCOUNT_GROUP,
                "channel": "matrixmedia",
                "title": title,
                "tags": list(KOREAN_PUBLISH_TAGS),
                "short_title": "韩语单词怎么说",
                "platforms": list(MATRIXMEDIA_VIDEO_PLATFORMS),
    ...
    next_action = (
        f"向用户展示每段成片路径、标题、标签和账号组（中文 YouTube「{CHINESE_PUBLISH_ACCOUNT_GROUP}」以及 Facebook/Instagram Reels、韩语「{KOREAN_PUBLISH_ACCOUNT_GROUP}」），等用户看过成品并明确确认后再发布。"
        "未确认不得调用 language_learning_publish，也不得调矩媒。"
        "确认后：有中文则 language_learning_publish(manifest_path, publish_confirmed=true)，会发 YouTube 和 Meta Reels；"
        f"有韩语则用 MatrixMedia MCP：list_accounts 筛 phone 等于「{KOREAN_PUBLISH_ACCOUNT_GROUP}」；"
        "未登录的抖音/视频号先 login 把二维码发给用户扫，再用 login_status 等到 success。"
        "平台 dy/ks/blbl/bjh/tt/sph，跳过掘金、番茄、小红书。"
        "每个账号每段视频调用一次 publish_video：file=output_path，title=成片标题，bt2=short_title，"
        f"tags 把四个标签拼成一个字符串，phone={KOREAN_PUBLISH_ACCOUNT_GROUP}。"
        "禁止复制改名为 publish.mp4，禁止用本地脚本代替矩媒 MCP。"
    )
    return {
        **video_result,
        "status": status,
...
def _youtube_channels(group_name: str) -> list[dict]:
    ...
    matched = [
        ...
        if str(item.get("channel_id") or "").strip() == CHINESE_YOUTUBE_CHANNEL_ID
    ]
    if not matched:
        titles = {str(group_name or "").strip(), CHINESE_PUBLISH_ACCOUNT_GROUP}
        matched = [
            item for item in accounts
            if str(item.get("channel_title") or "").strip() in titles
        ]
    if not matched:
        token_name = f"{CHINESE_YOUTUBE_CHANNEL_ID}.json"
        raise PublishError(
            f"YouTube 账号不存在：需要频道 {CHINESE_YOUTUBE_CHANNEL_ID}（{CHINESE_PUBLISH_ACCOUNT_GROUP}）。"
            f"请确认 data/youtube/tokens/{token_name} 已授权",
            {
                "account_group": group_name,
                "channel_id": CHINESE_YOUTUBE_CHANNEL_ID,
...
def _publish_chinese_meta(item: dict, run_id: str, platforms: list[str]) -> list[dict]:
    ...
    for index, video in enumerate(item["videos"], 1):
        ...
        except (MetaToolError, StorageError) as error:
            results.append({
                "channel": "meta",
                "video": video,
                "success": False,
...
def _publish_chinese_youtube(item: dict, *, include_youtube: bool, run_id: str, meta_platforms: list[str]) -> dict:
    channels = _youtube_channels(item["account_group"]) if include_youtube else []
...
def publish_vocabulary_videos(manifest_path: str | Path, publish_confirmed: bool) -> dict:
    ...
    korean_next = (
        f"用户已确认。韩语请用 MatrixMedia MCP：list_accounts 筛 phone 等于「{KOREAN_PUBLISH_ACCOUNT_GROUP}」；"
        "未登录的抖音/视频号先 login 把二维码发给用户扫，再用 login_status 等到 success。"
        "平台 dy/ks/blbl/bjh/tt/sph，跳过掘金、番茄、小红书。"
        "每个账号每段视频调用一次 publish_video：file=output_path，title=成片标题，bt2=short_title，"
        f"tags 把四个标签拼成一个字符串，phone={KOREAN_PUBLISH_ACCOUNT_GROUP}。"
    ...
    return {
        ...
        "published": published,
        "matrixmedia_items": matrixmedia_items,
        "next_action": korean_next if matrixmedia_items else "",
    }

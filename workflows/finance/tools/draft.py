...
from ..workflow import (
    _compose_finance_tts,
    _finance_storyboard_prompt,
    _parse_metadata,
    _read_prompt,
...
def save_draft(
    ...
    if draft_path is not None:
        ...
        record = update(database, WORKFLOW_ID, normalized_topic, TOPIC_DEDUPLICATION_DAYS)
...
def prepare_storyboard(draft_path: str | Path, *, user_confirmed: bool) -> dict:
    """用户确认稿件后生成 TTS，并返回让当前 Agent 生成分镜的完整提示。"""
    ...
    tts_result = _compose_finance_tts(str(draft["article"]), cache_root)
    ...
    context = {
        ...
        "next_tool": "finance_prepare_images",
    }
...
def finish_video(
    ...
    *,
    ...
    """使用当前 Agent 已提交的图片完成视频，并停在发布确认节点。"""
    if user_confirmed is not True:
        raise ConfirmationRequiredError("必须先获得用户对完整稿件的明确确认")
    resolved_draft, draft = _load_json(draft_path, "待确认稿件")
...
def clear_finance_run(run_id: str, *, confirmed: bool) -> dict:

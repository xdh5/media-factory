...
4. 为 GitHub Pages 发布看板设置六位数字秘密 `DASHBOARD_PIN`。看板只读接口为 `GET /v1/dashboard/records`；下载使用 `POST /v1/dashboard/download`，浏览器把 `url` 和 `pin` 作为 HTTPS 表单提交给 Worker，由浏览器下载器直接接收 Worker 回源的 R2 流，不在页面内缓存完整视频。兼容的 GET 下载接口继续使用 `X-Dashboard-Pin`。Worker 必须先验证 PIN 才查询 D1 或回源产物，无 PIN 或错误 PIN 返回 401。下载只允许数据库里已有的 `r2_url`，并以 `Content-Disposition: attachment` 强制保存文件。跨域读取仅允许 `https://xdh5.github.io`，响应不会返回本地路径、账号凭据或数据 API Token。

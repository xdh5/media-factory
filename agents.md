...
6. 财经与语言学习合并为 `weekly-production` Workflow：每周五北京时间 12:00 串行生产下周周一至周日内容；每个计划日发布日前先查 D1 去重，已有成片或发布记录则跳过该日。财经只生产并交付 R2；语言生产完成后自动四平台预约发布，失败时同日内重试最多 3 次。手动触发时可填写下周起始周一 `YYYY-MM-DD`，留空自动计算。发布 Workflow 仅允许手动触发，用于补发或重试。

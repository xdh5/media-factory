...
## MCP 调用要求
1. 已经提供 MCP 的工作流只能通过对应 MCP Tool 调用，禁止创建或运行临时 Python、PowerShell、批处理或其他脚本直接导入工作流内部函数。
2. MCP 未连接、进程退出或调用超时时，必须停止当前工作流并向用户报告具体错误；禁止绕过 MCP 直接读写数据库、缓存、任务文件或调用内部实现来继续执行。
3. 调试 MCP 只允许检查配置、启动状态和服务日志，不得用本地脚本代替 MCP 完成业务步骤。
...
## 工作流
1. 每个工作流必须通过自身 MCP 对外；耗时步骤必须走共用后台任务 `enqueue_job` / `get_job`（工作流 MCP 封装为 `*_get_job`），立即返回 `job_id`，禁止在 MCP 工具里同步卡住配音、生图提交、视频合成等长步骤。
2. 每个工作流必须使用共用话题历史 `get_topic` / `update` 做去重与占坑，禁止另起一套主题库。
3. 发布与清缓存分开：发布成功后必须再向用户确认是否删除本次生产文件，确认后调用共用 `clear_run`（工作流 MCP 封装为 `*_clear_run`）；不得把清缓存写进发布工具的返回或自动删除。

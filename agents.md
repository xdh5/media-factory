...
6. 财经与语言学习合并为每周生产 Workflow：每周六北京时间 12:00 串行生产下周周一至周日的财经与语言内容；单日或单个产品失败不得阻断后续日期。禁止用 D1 健康检查触发自动补发或整批重试，避免平台已接收但状态延迟时重复上传。财经只生产并交付 R2；语言生产完成后自动在 YouTube、TikTok、Facebook、Instagram 预约发布。YouTube 只认官方 API 的单次请求结果：完整上传或预约并返回 `video_id` 才记为成功，抛错或缺少 `video_id` 记为失败。TikTok、Facebook、Instagram 调用 Zernio；Zernio 请求成功并返回 `post_id` 与成功或预约状态后，立即把标题、发布时间、平台、账号、分段及外部 ID 写入现有 `publication_records`，请求失败则不写入。原每日生产命令保留但不再配置定时调度；发布 Workflow 仅允许手动触发。

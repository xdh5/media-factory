...
from ._constants import (
    ...
    VIDEO_CRF,
    VIDEO_FPS,
    VIDEO_IMAGE_LOOP_PAD_SECONDS,
    VIDEO_MOTION_INPUT_SCALE,
    VIDEO_MOTION_WORK_SCALE,
    VIDEO_PIXEL_FORMAT,
    VIDEO_PROBE_TIMEOUT_SECONDS,
    VIDEO_PRESET,
    VIDEO_RENDER_MIN_TIMEOUT_SECONDS,
    VIDEO_RENDER_TIMEOUT_PER_SECOND,
    VIDEO_TUNE_STILLIMAGE,
)
from ._errors import (
    FFmpegNotFoundError,
...
def _write_timeline_ass(path: Path, cues: list[dict], size: str) -> None:
    """按整片时间轴写出 ASS，供成片阶段一次烧录。"""
    if not cues:
        raise InvalidParameterError("subtitle", "没有可烧录的字幕")
    canvas = None
    ...
    for cue in cues:
        ...
        if canvas is None:
            canvas = style
        style_lines.append(
            f"Style: S_{language},{style['font']},{style['font_size']},"
            "&H00FFFFFF,&H00FFFFFF,&H00101010,&H00000000,1,0,0,0,100,100,0,0,1,"
    ...
    for cue in cues:
        ...
        events.append(
            ...
            f"S_{language},,0,0,0,,{safe_text}"
        )
    if canvas is None or not events:
        raise InvalidParameterError("subtitle", "没有可烧录的字幕文本")
    header = f"""[Script Info]
ScriptType: v4.00+
...
def _encode_video_args(*, still_image: bool) -> list[str]:
    ...
    if still_image:
        args.extend(["-tune", VIDEO_TUNE_STILLIMAGE])
    return args
...
def _motion_filter(
    ...
    *,
    ...
    camera_input_width = width * VIDEO_MOTION_INPUT_SCALE
    camera_input_height = height * VIDEO_MOTION_INPUT_SCALE
    work_width = width * VIDEO_MOTION_WORK_SCALE
    work_height = height * VIDEO_MOTION_WORK_SCALE
    return ",".join([
        ...
        f"d=1:s={work_width}x{work_height}:fps={VIDEO_FPS}",
...
def render_shot(
    ...
    *,
    ...
    image_loop_seconds = effective_duration + VIDEO_IMAGE_LOOP_PAD_SECONDS
    command = [
        ffmpeg, "-y",
    ...
    if audio:
        if effective_audio_start:
            command.extend(["-ss", f"{effective_audio_start:.6f}"])
        if audio_end is not None:
            command.extend(["-to", f"{effective_audio_end:.6f}"])

...
from .select_subtitle import _parse_size, select_subtitle
...
def _write_ass(path: Path, text: str, duration: float, language: str, size: str) -> None:
    style = select_subtitle(language, size)
...
def _motion_filter(motion: dict, duration: float, width: int, height: int) -> str:
    values = {
    ...
    progress = "1" if frames == 1 else f"on/{frames - 1}"
    ...
    camera_input_width = width * 4
    camera_input_height = height * 4
    camera_work_width = width * 2
    camera_work_height = height * 2
    return ",".join([
        ...
        f"d=1:s={camera_work_width}x{camera_work_height}:fps={VIDEO_FPS}",

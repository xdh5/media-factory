...
def _read_config() -> dict:
    try:
        ...
        raise QwenConfigurationError(f"千问配置文件不存在：{CONFIG_PATH}") from exc
    except (OSError, json.JSONDecodeError) as exc:
        raise QwenConfigurationError(f"读取千问配置失败：{CONFIG_PATH}。{exc}") from exc
    if not isinstance(payload, dict):
        raise QwenConfigurationError("千问配置必须是 JSON 对象")
    return payload
...
def get_qwen_configuration() -> dict:
    ...
    base_url = os.getenv(DASHSCOPE_BASE_URL_ENV, "").strip() or str(
        configured.get("base_url") or DEFAULT_BASE_URL
    ).strip()
    model = os.getenv(DASHSCOPE_MODEL_ENV, "").strip() or str(
        configured.get("model") or DEFAULT_MODEL
    ).strip()
    thinking_value = os.getenv(DASHSCOPE_ENABLE_THINKING_ENV)
    enable_thinking = _boolean(
        configured.get("enable_thinking", False) if thinking_value is None else thinking_value,
        name=DASHSCOPE_ENABLE_THINKING_ENV,
    )
    try:
        timeout_seconds = int(configured.get("timeout_seconds", DEFAULT_TIMEOUT_SECONDS))
        temperature = float(configured.get("temperature", DEFAULT_TEMPERATURE))
    except (TypeError, ValueError) as exc:
        raise QwenConfigurationError("千问 timeout_seconds 或 temperature 配置格式错误") from exc
    if not base_url.startswith("https://"):
        raise QwenConfigurationError("千问 base_url 必须使用 https://")
    if not model:
        raise QwenConfigurationError("千问 model 不能为空")
...
def generate_text(
    ...
    *,
    ...
    """使用配置中的 qwen3.8-max 生成文本并返回 Token 用量。"""

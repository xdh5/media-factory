...
def parse_storyboard(value: str, timeline: list[dict]) -> list[dict]:
    ...
    for row in value.splitlines():
        ...
        if len(fields) != 5 or fields[2].upper() != "VIDEO":
            continue
...
def load_prepared_tts(cache_root: str | Path) -> dict:
    ...
    if not tts_path.is_file():
        raise WorkflowStepError(
            "分镜上下文没有完整配音文件。请重新调用 psychology_quiz_prepare_storyboard",
            {"tts_path": str(tts_path)},
        )
    if not isinstance(timeline, list) or not timeline:
        raise WorkflowStepError("分镜上下文缺少有效 timeline，请重新调用 psychology_quiz_prepare_storyboard")
    return {
        "tts_path": tts_path,
        "timeline": timeline,
